...
🚀 **StarSpark** 是一个 GitHub Trending 热门项目爬取与管理工具，支持自动抓取 GitHub 热门仓库数据、中文翻译、本地持久化存储以及可选的项目源码包下载。
...
3. 按配置的周期类型逐一爬取 GitHub Trending 榜单
4. 自动翻译项目描述为中文
5. 将项目信息与指标快照写入数据库
6. 若开启 `auto_download`，则通过镜像下载项目 ZIP 包
...
| 模块 | 说明 |
|------|------|
| [`config.py`](config.py) | 使用 `dataclass` 将 YAML 配置映射为强类型对象 |
